...
import '../isar/isar_pulse_event_entity.dart';
...
class IsarPulseEventRepository implements PulseEventRepository {
  ...
  static final _metricByName = {
    for (final e in PulseMetric.values) e.name: e,
  };
  ...
  static LocalDate _localDateFromString(String s) {
    final parts = s.split('-');
    if (parts.length != 3) throw FormatException('LocalDate format YYYY-MM-DD: $s');
    return LocalDate(
  ...
  IsarPulseEventEntity _toEntity(PulseEvent event) {
    ...
    final localDateStr = _localDateToString(LocalDate.fromDateTime(occurredAtUtc));
        ...
        final payload = {
          'observation': {
            ...
            'observedAt': {
              ...
              'deviceTimeIso': obs.observedAt.deviceTime.toUtc().toIso8601String(),
            },
            'note': obs.note,
          },
        };
  ...
  MetricLogged? _parseMetricLogged(IsarPulseEventEntity entity) {
    try {
      ...
      final observedAt = ObservedAt(localDate: localDate, deviceTime: deviceTime);
