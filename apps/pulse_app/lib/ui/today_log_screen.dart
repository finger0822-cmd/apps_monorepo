...
class TodayLogScreen extends StatefulWidget {
  const TodayLogScreen({super.key, required this.deps});
...
class _TodayLogScreenState extends State<TodayLogScreen> {
  ...
  /// 5項目の値（順序は PulseMetric.all に準拠: 気力・集中・疲れ・気分・眠気）
  ...
  Future<void> _onNextPressed() async {
    ...
    try {
      await widget.deps.usecase.upsertForDate(
        date: DateTime.now(),
        energy: _values[0],
        focus: _values[1],
        fatigue: _values[2],
                          ...
                          children: [
                            ...
                            Align(
                              ...
                              child: GestureDetector(
                                onTap: canProceed && !_saving ? _onNextPressed : null,
...
/// 1項目の入力行。項目名は長押しで説明シートを表示、1〜5の選択は円ボタンタップのみで競合しない。
