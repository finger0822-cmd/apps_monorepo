  ...
  Future<void> _handleSubmit({String source = 'unknown'}) async {
    ...
    if (state.submitStatus == SubmitStatus.success && mounted) {
      ...
      if (Platform.isWindows) {
        ...
        // 1.5秒後に自動的にリセットして入力画面へ戻す（Timerで管理してdisposeでキャンセル可能にする）
        _sentResetTimer = Timer(const Duration(milliseconds: 1500), () {
          _onSentResetTimer(sessionId: submitSessionId);
        });
        CrashLogger.logDebug('[NowSheet] Windows: _sentResetTimer started (1500ms) sessionId=$submitSessionId');
        return;
      }
